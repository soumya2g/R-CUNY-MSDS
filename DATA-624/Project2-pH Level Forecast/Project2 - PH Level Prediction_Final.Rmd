---
title: "DATA 624 - Project2 - PH Level Prediction"
author: "Soumya Ghosh, Jose Mawyin & Randy Thompson"
...
head(training_df) %>% kable() %>% kable_styling(bootstrap_options = c("striped", "hover", "condensed", "responsive")) %>% scroll_box(width="100%",height="300px")
```
...
```{r}
# Excluding the PH Level target variable from the data frame to focus on predictors only

studentPredsDF <- training_df[,-26]
...
training_df %>%
  ...
  labs(y='Frequency', x = "PH of Beverage", title='PH Levels in Dataset')
```
...
studentPredsDF <- studentPredsDF[2:ncol(studentPredsDF)]

par(mfrow = c(7,5), cex = .5)
for(i in colnames(studentPredsDF)){
boxplot(studentPredsDF[,i], xlab = names(studentPredsDF[i]),
  main = names(studentPredsDF[i]), col="grey", ylab="")
}
```
...
```{r}
rf_fm <- rf_grid$finalModel
imp <- as.data.frame(varImp(rf_fm))
...
test_df$Air.Pressurer_bin <- ifelse(test_df$Air.Pressurer > 145, 1, 0)
test_df$Balling.Lvl_bin <- ifelse(test_df$Balling.Lvl > 2, 1, 0)
test_df$Balling_bin <- ifelse(test_df$Balling > 2.5, 1, 0)
test_df$Density_bin <- ifelse(test_df$Density > 1.25, 1, 0)
test_df$Hyd.Pressure1_bin <- ifelse(test_df$Hyd.Pressure1 == 0, 0, 1)
test_df$Hyd.Pressure2_bin <- ifelse(test_df$Hyd.Pressure2 == 0, 0, 1)
test_df$Hyd.Pressure3_bin <- ifelse(test_df$Hyd.Pressure3 == 0, 0, 1)
test_df$Mnf.Flow_bin <- ifelse(test_df$Mnf.Flow > 50, 0, 1)
#=========
#Imputing using the pmm mehtod, creating 5 dataframes, do not print process
imp_training <- mice(test_df, m=5, method = "pmm",print =  FALSE)
# Get complete data ( 3rd out of 5)
test_df <- complete(imp_training,3)
rf_predictions <- rf_grid %>% predict(test_df)
rf_predictions <- as.data.frame(rf_predictions)
test_df$PH <- round(rf_predictions$rf_predictions,1)
write.csv(x=test_df, file="GroupSRJ_Predictions.csv")
```
